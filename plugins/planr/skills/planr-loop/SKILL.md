...
- Codex: apply the native Codex preset to generate the repository-owned Sol/Terra/Luna roles and `.codex/skills/planr-native-routing/SKILL.md`. The parent is `planr-sol-medium`; dispatch exploration/research to `planr-terra-medium`, normal implementation to `planr-terra-high`, mechanical work to `planr-luna-xhigh`, review to `planr-sol-high`, and explicitly selected moonshot planning to `planr-sol-ultra`. Every child call names the generated role, for example `spawn_agent({ agent_type: "planr-terra-high", fork_turns: "none", ... })`. Use `fork_turns: "none"` or an evidenced positive bounded count, never `fork_turns: "all"`; the generated role owns model and effort, and Codex routes do not use fallback chains.
