...
The recommended combination. Install the plugin, initialize the repository, and apply the native Codex preset once:
...
The `/goal` PM follows `.codex/skills/planr-native-routing/SKILL.md`: exploration uses `planr-terra-medium`, normal implementation uses `planr-terra-high`, mechanical work uses `planr-luna-xhigh`, and independent review uses `planr-sol-high`. Every child call names `agent_type` and uses `fork_turns: "none"` or an evidenced positive bounded count; the generated role TOML is the sole model/effort owner. Codex Automations use the same starter line.
...
A goal run has distinct roles with different intelligence needs, so they should not all run on the same model tier:

- **Driver**: Sol Medium owns normal orchestration and synthesis.
- **Explorer**: Terra Medium owns repository exploration and research.
- **Worker**: Terra High owns normal implementation and testing.
- **Mechanical worker**: Luna xHigh owns bounded, checklist-driven work.
- **Reviewer**: Sol High owns the independent truth gate.
- **Moonshot planner**: Sol Ultra is explicit-only and never the balanced default.

Where each host configures its tiers:

| Host | Driver | Worker | Configured in |
| --- | --- | --- | --- |
| Codex | generated `planr-sol-medium` driver | generated Terra High or Luna xHigh role selected by `agent_type` | `.codex/agents/planr-*.toml` from the applied preset |
...
Claude and Cursor retain their independent host contracts. Codex uses exact GPT-5.6 Sol/Terra/Luna role files generated by the selected preset; change tiers by previewing and applying a different preset, not by adding call-site overrides.

The applied Codex preset writes the agent registry and repository role files together. Its routes have no fallback chain: each work type maps directly to one generated `agent_type`, while the matching role TOML owns model and effort. See [Model Routing](MODEL_ROUTING.md).
...
- **Codex:** native Codex rejects `fork_turns = "all"` when `agent_type` selects a role with model or effort overrides, so use `fork_turns = "none"` or an evidenced positive partial fork. The role registry is loaded once at session start ([#26408](https://github.com/openai/codex/issues/26408)): after applying changed `.codex/agents/*.toml` files, restart the session so the generated definitions are live. Spawn the generated role on a trivial item and confirm the child metadata shows the pinned model and effort with a non-null `agent_path`.
