...
`agents preset apply <policy-path-or-id> --binding <binding-path-or-id>` composes a provider-neutral Usage Policy v1 file with a strict, versioned host binding. Embedded id pairs report `pack.status = safe`; explicit file inputs remain allowed custom compositions and receive a clear non-safe warning even if they reuse built-in ids. Preview is the default and reports compatibility, per-role permission additions, verification age, provenance, content hashes, every normalized repository-relative target, create/unchanged/conflict state, and a redacted old/proposed configuration projection. `--confirm` writes exactly the conflict-free preview to repository-local artifacts and never overwrites different existing files. A binding containing Codex profiles first reads the installed version from `codex --version`, requiring 0.144.0 or newer, then requires the live-host adapter and hash-pinned trusted-telemetry collector to prove native-v2 dispatch and every requested agent type/model/effort. Unsigned caller capability assertions are not part of either the CLI or MCP contract and no capability claims are persisted in the lock. Host-specific bindings require every profile client to match the host; explicit cross-client mappings use `host = "mixed-host"`. Binding schema and repository-boundary details: [Preset Composition](PRESET_COMPOSITION.md).
...
`agents init` atomically writes the canonical native Codex Sol/Terra/Luna registry, every matching repository-local `.codex/agents/*.toml` role, and the native routing skill. All targets are prevalidated against the repository boundary and unrelated existing artifacts are never overwritten; a successful init is immediately dispatchable. Repeatable flags and the wizard are for non-Codex Claude, Cursor, and generic MCP pools. Custom Codex profile specs are not accepted because the embedded `codex-openai` binding is the single owner of registry and role content. `agents list` and `agents check` inspect the registry, while pick packets carry the resolved profile, agent type, host, model, effort, tier, skill, route chain, and matched selector. Full guide: [Model Routing](MODEL_ROUTING.md).
