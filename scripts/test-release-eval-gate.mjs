...
if (args.includes('suite-check')) {
  ...
  console.log(JSON.stringify({ok:true,object:{run:{id,suite_digest:process.env.FAKE_SUITE,subject_revision:scenario === 'mismatched' ? 'sha256:'+'b'.repeat(64) : revision,status:'success',invalidated_by:null,cases:[{attempts:[attempt]}]}}}));
} else process.exit(4);
`);
...
const baseReceipt = {
  schema_version: "planr.release-eval-receipt.v1",
