...
export function runVerification({
  ...
  const receipt = {
    ...
    selection: {
      ...
      liveVerification: selection.liveVerification,
    },
...
export function verifyReceipt(receipt, { selection, repoRoot = process.cwd(), artifactPaths = DEFAULT_ARTIFACTS, receiptPath } = {}) {
  ...
  assertEqual(canonicalJson(receipt.selection.liveVerification), canonicalJson(selection.liveVerification), "live verification selection mismatch");
...
function validateReceiptShape(receipt) {
  ...
  assertKeys(receipt.selection, ["escalatedToFull", "liveVerification", "matchedPathClasses", "profile", "selectedGates"], "selection");
...
function assertSelection(selection) {
  ...
  if (
    !Array.isArray(selection.changes)
    || !Array.isArray(selection.selectedGates)
    || typeof selection.liveVerification?.browser !== "boolean"
