...
description: Complete Planr 1.10.0-alpha.2 tools/list inventory with every property, type, description, required field, enum/default declaration, and additionalProperties rule.
---
...
```json
{
  ...
  "properties": {
    ...
    "plan": {
      "description": "Only lease items belonging to this plan (plan id), so plan-scoped goal runs stay inside their contract",
      "type": "string"
    },
    "work_type": {
      "description": "Only lease items of this work type (e.g. review for checkers, code for makers)",
      "type": "string"
    }
  },
  "required": [],
...
```json
{
  ...
  "properties": {
    "apply": {
      "description": "Apply the rebind; omit or false for preview",
      "type": "boolean"
    },
    "input": {
      "description": "planr.evidence.rebind.v1 payload with plan_id, manifest_id, obligations[], and preview_digest for apply",
      "type": "object"
    }
  },
  "required": [
...
```json
{
  ...
  "properties": {
    ...
    "attachment_kind": {
      "description": "log, review, or artifact",
      "type": "string"
    },
    "item_id": {
...
```json
{
  ...
  "properties": {
    "close_target": {
      "description": "With verdict complete: also close the reviewed target item (requires a completion log on the target)",
      "type": "boolean"
    },
    "findings": {
    ...
    "reviewer": {
      "description": "Reviewer identity recorded on the review log, artifact, and event (defaults to the worker id)",
      "type": "string"
    },
    "verdict": {
      "description": "Verdict: complete, partial, failed, or unclear",
      "type": "string"
    }
  },
  "required": [
