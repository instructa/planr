...
| `reviews` | Historical read-only relationship; new review authority is represented by ReviewGate state. |
...
The direct item lifecycle is `pending → ready → picked → closed`; `running` is an optional explicit work state. Evidence-backed `planr done` settles the outcome through its FeatureRun and closes the item. When materiality or structured escalation requires review, a separate durable ReviewGate becomes pending while the FeatureRun waits for an independent accepted attempt. Other item states are `blocked`, `failed`, `cancelled`, and `closed_partial`.
