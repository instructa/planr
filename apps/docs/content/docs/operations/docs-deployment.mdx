...
Run `PLANR_DOCS_URL=https://planr.so pnpm docs:verify-release-live` after deployment and record the command output with the release evidence.

## Promote

Promote only after static, semantic, live-route, and search evidence passes. Record the Alchemy stage, Worker identity, deployment output, commit, and last known-good commit needed by [Rollback](/docs/operations/rollback). Do not run `pnpm docs:destroy` as rollback.
