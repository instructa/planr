...
# Declare the client you will actually dispatch on: a loop running inside
# one host dispatches that host's subagents, so a `codex` profile is only
# honest when the driver really spawns a Codex process (`codex exec`).
[profiles.driver]
client = "cursor"
model = "fable-5"
effort = "high"
...
/// Renders the compiled spec as a commented registry in the same shape
/// as the static scaffold. Pure so the flag path and the wizard share it.
fn build_registry(spec: &InitSpec) -> String {
    let mut out = String::from(
        "# Planr agent profile registry: advisory model routing for pick packets.\n\
         ...
         #\n\
         # Declare the client you will actually dispatch on: a loop running\n\
         # inside one host dispatches that host's subagents, so a `codex`\n\
         # profile is only honest when the driver really spawns a Codex\n\
         # process (`codex exec`). Runs record the observed host, and a\n\
         # deviation from the declared client is flagged in `trace item`.\n",
    );
...
impl App {
    pub(crate) fn agents_init(&self, args: AgentsInitArgs) -> Result<()> {
        ...
        self.emit(
            ...
            format!(
                "wrote {REGISTRY_RELATIVE_PATH} ({})\nnext: `planr agents check` to validate, then `planr install <client> --force` to render the host role files with the pins",
                if mode == "flags" {
                    "generated from the flag specs".to_string()
                } else {
                    "cost-tiering starter: premium driver, standard implementer, budget helper"
                        .to_string()
                }
            ),
        )
...
mod tests {
    ...
    fn full_spec_compiles_and_builds_a_zero_warning_registry() {
        let spec = compile_specs(&args(
            &[
                ...
                "backender=codex/gpt-5.5@xhigh#standard",
            ],
        ...
        assert!(
            content.contains("Declare the client you will actually dispatch on"),
            "builder output must carry the client-honesty rule"
        );
    ...
    fn specs_fail_closed_with_grammar_in_the_message() {
        for (args, needle) in [
            ...
            (
                args(&["a=cursor/m", "a=codex/m"], &[], &[], None),
