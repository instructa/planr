...
//! canonical Codex registry/artifact initialization plus the non-Codex
//! flag-driven spec builder and interactive wizard — kept out of
...
fn parse_profile_spec(spec: &str) -> Result<ProfileSpec> {
    ...
    if client.trim() == "codex" {
        bail!(
            "custom Codex profiles are preset-owned; use `planr agents preset apply <policy> --binding codex-openai`"
        );
    }
    let parts = [
        ("id", id),
...
/// Renders a non-Codex pool specification. Pure so flags and the wizard share it.
fn build_registry(spec: &InitSpec) -> String {
    let mut out = String::from(
        "# Planr agent profile registry: advisory model routing for pick packets.\n\
         ...
         #\n\
         # Custom pools support Claude, Cursor, and generic MCP. Native Codex\n\
         # profiles are generated only by `agents preset apply` so agent_type,\n\
         # model, effort, and repository role files cannot drift.\n",
    );
...
const CLIENTS: [&str; 3] = ["claude-code", "cursor", "generic-mcp"];
...
impl App {
    pub(crate) fn agents_init(&self, args: AgentsInitArgs) -> Result<()> {
        ...
        let (mode, content, wizard_spec) = if args.interactive {
            ...
            (
                "scaffold",
                crate::app::presets::canonical_codex_registry_content()?,
                None,
            )
        };
        ...
        if mode == "scaffold" {
            self.initialize_canonical_codex_agents(args.force)?;
        } else {
        ...
        self.emit(
            json!({
                ...
                "created": true,
                "mode": mode,
                "next": if mode == "scaffold" {
                    json!(["planr agents check", "native Codex roles are ready"])
                } else {
                    json!(["planr agents check", "planr install <client> --force"])
                },
            }),
            if mode == "scaffold" {
                format!("wrote {REGISTRY_RELATIVE_PATH} and canonical repository-local Codex roles atomically; native dispatch is ready")
            } else {
                format!("wrote {REGISTRY_RELATIVE_PATH} from the non-Codex pool specification\nnext: `planr agents check`, then `planr install <client> --force`")
            },
        )
    }
...
mod tests {
    ...
    fn full_spec_compiles_and_builds_a_zero_warning_registry() {
        let spec = compile_specs(&args(
            &[
                ...
                "backender=cursor/fable-code@high#standard",
            ],
        ...
        assert!(
            content.contains("Native Codex") && !content.contains("codex exec"),
            "custom-pool builder must direct Codex ownership to preset apply"
        );
    ...
    #[test]
    fn custom_codex_specs_defer_to_the_native_preset_owner() {
        let error = compile_specs(&args(
            &["worker=codex/gpt-5.6-terra@high#standard"],
            &[],
            &[],
            None,
        ))
        .unwrap_err()
        .to_string();
        assert!(error.contains("preset-owned"));
        assert!(error.contains("codex-openai"));
    }
