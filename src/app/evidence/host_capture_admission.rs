...
impl PendingHostCaptureAdmission {
    pub(crate) fn validate_pending(&self) -> Result<()> {
        for digest in [
            &self.sealed_run_index_digest,
            &self.normalized_capture_digest,
        ] {
        ...
        if !self.execution_binding.is_object()
            || !self.manifest.is_object()
            || !self.instance.is_object()
        {
...
pub(crate) fn insert_pending(conn: &Connection, value: &PendingHostCaptureAdmission) -> Result<()> {
    ...
    if changed != 1 {
        bail!("host capture admission insert did not create one record");
    }
    Ok(())
}
...
pub(crate) fn load_pending(
    conn: &Connection,
    digest: &str,
) -> Result<Option<PendingHostCaptureAdmission>> {
