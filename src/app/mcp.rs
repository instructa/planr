...
impl App {
    ...
    pub(crate) fn mcp_tool_call(&self, params: Value) -> Result<Value> {
        ...
        match name {
            ...
            "planr_trace_item" => Ok(mcp_json(
                self.trace_item_value(required_arg(&args, "item_id")?)?,
            )),
            "planr_agents_list" => Ok(mcp_json(self.agents_list_value()?.0)),
            "planr_presets_list" => Ok(mcp_json(crate::preset_catalog::catalog_value())),
            "planr_preset_evaluate" => {
                let at_unix = match args.get("at_unix") {
                    Some(value) => Some(
                        value
                            .as_u64()
                            .ok_or_else(|| anyhow!("at_unix must be an unsigned integer"))?,
                    ),
                    None => None,
                };
                let host = match args.get("host") {
                    Some(value) => Some(
                        value
                            .as_str()
                            .ok_or_else(|| anyhow!("host must be a string"))?,
                    ),
                    None => None,
                };
                let live_host = match args.get("live_host_command") {
                    Some(value) => {
                        let executable = value
                            .as_str()
                            .ok_or_else(|| anyhow!("live_host_command must be a string"))?;
                        let command_args = match args.get("live_host_args") {
                            Some(value) => value
                                .as_array()
                                ...
                                .map(|value| {
                                    value.as_str().map(ToOwned::to_owned).ok_or_else(|| {
                                        anyhow!("live_host_args entries must be strings")
                                    })
                                })
                                .collect::<Result<Vec<_>>>()?,
                            None => Vec::new(),
                        };
                        Some(crate::preset_eval::LiveHostCommand {
                            executable: PathBuf::from(executable),
                            args: command_args,
                        })
                    }
                    None => {
                        if args.get("live_host_args").is_some() {
                            return Err(anyhow!("live_host_args requires live_host_command"));
                        }
                        None
                    }
                };
                let trusted_telemetry = match (
                    args.get("trusted_telemetry_signer"),
                    args.get("trusted_telemetry_collector"),
                ) {
                    (Some(signer), Some(collector)) => {
                        if live_host.is_none() {
                            return Err(anyhow!("trusted telemetry requires live_host_command"));
                        }
                        Some(crate::preset_eval::TrustedTelemetryCommand {
                            ...
                            signer_id: signer
                                .as_str()
                                .ok_or_else(|| {
                                    anyhow!("trusted_telemetry_signer must be a string")
                                })?
                                .to_string(),
                            collector: PathBuf::from(collector.as_str().ok_or_else(|| {
                                anyhow!("trusted_telemetry_collector must be a string")
                            })?),
                        })
                    }
                    (None, None) => None,
                ...
                let (value, _) =
                    self.preset_evaluation_value(at_unix, host, live_host, trusted_telemetry)?;
                Ok(mcp_json(value))
            }
            "planr_preset_apply" => Ok(mcp_json(
                self.preset_apply_value(
                    &PathBuf::from(required_arg(&args, "policy")?),
                    &PathBuf::from(required_arg(&args, "binding")?),
                    args.get("confirm")
            ...
            "planr_preset_registry_verify" => {
                Ok(mcp_json(self.preset_registry_verify_mcp_value(&args)?))
            }
            "planr_preset_registry_import" => {
                Ok(mcp_json(self.preset_registry_import_mcp_value(&args)?))
            }
            "planr_preset_registry_list" => {
                Ok(mcp_json(self.preset_registry_list_mcp_value(&args)?))
            }
            "planr_policy_show" => Ok(mcp_json(self.policy_show_value().0)),
            "planr_policy_check" => Ok(mcp_json(self.policy_check_value()?.0)),
            "planr_policy_admit" => Ok(mcp_json(
                self.policy_admit_value(args.get("request").cloned().unwrap_or_else(|| json!({})))?,
            )),
