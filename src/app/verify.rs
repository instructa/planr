...
impl App {
    pub(crate) fn try_raw_dispatch(&self, raw_args: &[String]) -> Result<bool> {
        ...
        match args[0].as_str() {
            ...
            "plan" if args.get(1).map(String::as_str) == Some("audit") => {
    ...
    fn verify_point_add(&self, args: &[String]) -> Result<()> {
        let item_id = required_positional(args, "verify point add requires an item id")?;
        ...
        self.conn.execute(
            ...
            params![id.clone(), item.project_id.clone(), item.id.clone(), plan_id, source_type, source_id, kind.clone(), text, if required { 1 } else { 0 }],
        )?;
    ...
    fn verify_replay(&self, args: &[String]) -> Result<()> {
        let id = required_positional(args, "verify replay requires an evidence id")?;
    ...
    fn verify_run(&self, spec: RunSpec) -> Result<()> {
        ...
        let capture = run_command(&spec.command, &cwd);
        ...
        let capability_state = json!({
            ...
            "timeout_seconds_requested": spec.timeout_seconds,
        });
        self.conn.execute(
            ...
            params![run_id.clone(), item.project_id.clone(), item.id.clone(), worker_id(), client, spec.command.clone(), cwd_display.clone(), status, capture.exit_code, json!({"verification_evidence_id": evidence_id.clone(), "kind": spec.kind.clone()}).to_string()],
        )?;
        self.conn.execute(
            ...
            params![evidence_id.clone(), item.project_id.clone(), item.id.clone(), spec.point_id.clone(), run_id.clone(), spec.kind.clone(), spec.command.clone(), cwd_display, capture.exit_code, status, stdout.value.clone(), stderr.value.clone(), assertions.to_string(), json!([]).to_string(), capability_state.to_string(), capture.duration_ms, spec.replay_of.clone()],
        )?;
        if let Some(point_id) = spec.point_id.as_deref() {
        ...
        let tests = if matches!(spec.kind.as_str(), "test" | "unit" | "integration" | "e2e") { commands.clone() } else { serde_json::to_string(&Vec::<String>::new())? };
        ...
        self.conn.execute(
            ...
            params![log_id.clone(), item.project_id.clone(), item.id.clone(), run_id.clone(), format!("verification {status}: {} (exit {:?})", first_line(&spec.command), capture.exit_code), commands, tests, blocked],
        )?;
    ...
    fn plan_audit_ext(&self, plan_id: &str, flags: &AuditFlags) -> Result<Value> {
        ...
        clauses.push(json!({"clause": "verification_points", "pass": !flags.require_points || (!required.is_empty() && open_required.is_empty()), "required": flags.require_points, "total": points.len(), "required_points": required.len(), "passed_required_points": required.len().saturating_sub(open_required.len()), "open": open_required}));
        clauses.push(json!({"clause": "planr_executed_verification", "pass": !strong.is_empty(), "required": strict, "evidence": strong.clone(), "detail": if strict { "strict/autonomous audit requires Planr-executed pass evidence" } else { "Planr-executed evidence is preferred; default mode remains backwards compatible" }}));
        clauses.push(json!({"clause": "git_policy", "pass": git["pass"].as_bool().unwrap_or(false), "required": git_required, "policy": flags.git_policy.clone(), "evidence": git}));
        ...
        value["verification"] = json!({"points": points, "evidence": evidence, "strong_passes": strong.len()});
        if !holds {
    ...
    fn git_policy_value(&self, policy: &str) -> Result<Value> {
        ...
        if !self.root.join(".git").exists() || !command_exists("git") {
            return Ok(json!({"policy": policy, "pass": policy == "auto", "status": if policy == "auto" { "not_available" } else { "blocked" }}));
        }
        let output = StdCommand::new("git").arg("-C").arg(&self.root).args(["status", "--porcelain"]).output()?;
        ...
        Ok(json!({"policy": policy, "pass": pass, "status": if clean { "clean" } else { "dirty" }, "dirty_files": dirty}))
    }
}
...
fn parse_run_spec(args: &[String]) -> Result<RunSpec> {
    let item_id = required_positional(args, "verify run requires an item id")?;
...
fn required_positional(args: &[String], message: &str) -> Result<String> {
    args.first().filter(|value| !value.starts_with('-')).map(ToOwned::to_owned).ok_or_else(|| anyhow!(message.to_string()))
}
...
fn run_command(command: &str, cwd: &PathBuf) -> Capture {
    let start = Instant::now();
    let output = shell_command(command).current_dir(cwd).output();
    match output {
        Ok(output) => Capture { exit_code: output.status.code(), stdout: String::from_utf8_lossy(&output.stdout).into_owned(), stderr: String::from_utf8_lossy(&output.stderr).into_owned(), spawn_error: None, duration_ms: start.elapsed().as_millis() as i64 },
        Err(error) => Capture { exit_code: None, stdout: String::new(), stderr: String::new(), spawn_error: Some(error.to_string()), duration_ms: start.elapsed().as_millis() as i64 },
    }
}
...
fn evaluate(spec: &RunSpec, capture: &Capture, cwd: &PathBuf) -> (&'static str, Value) {
    ...
    for file in &spec.file_exists {
        ...
        if !pass { ok = false; }
        assertions.push(json!({"type": "file_exists", "path": path.to_string_lossy().to_string(), "pass": pass}));
    }
    let status = if ok { "pass" } else if capture.spawn_error.is_some() { "blocked" } else { "fail" };
...
fn parse_json_array(raw: Option<String>) -> Value { raw.and_then(|text| serde_json::from_str(&text).ok()).filter(Value::is_array).unwrap_or_else(|| json!([])) }
fn parse_json_or_null(raw: Option<String>) -> Value { raw.and_then(|text| serde_json::from_str(&text).ok()).unwrap_or(Value::Null) }
fn first_line(value: &str) -> String { value.lines().next().unwrap_or(value).chars().take(80).collect() }
...
fn audit_human(value: &Value) -> String {
    ...
    for clause in value["clauses"].as_array().into_iter().flatten() {
        ...
        out.push_str(&format!("{verdict} {}\n", clause["clause"].as_str().unwrap_or("clause")));
    }
    if value["holds"].as_bool().unwrap_or(false) { out.push_str("contract holds"); } else { out.push_str("contract open"); }
    out
}
